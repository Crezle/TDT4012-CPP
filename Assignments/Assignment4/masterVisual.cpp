...
{
	vector<char> cCode(code.begin(),code.end());
	int color = 0;
	for (int i = 0; i < size; i++) {
		color = static_cast<int>(cCode[i] - startLetter) + 1;
		mwin.vr.push_back(new Rectangle{Point{padX + padX*2*i,padY*3 + padY*2*round},padX,padY});
...
{
	for (int i = 0; i < size; i++) {
		mwin.vc.push_back(new Circle{Point{padX*9 + 2*radCircle*i,(padY*3) + padY/2 + padY*2*round},radCircle});
